...
public class UserManager {
	...
	public UserAccount login(UserAccount user) throws UserManagerException {
		...
		try {
			list = FacadeFactory.getFacade()
					.list(UserAccount.class, parameters);
		} catch (DataAccessLayerException e) {
			logger.error("Account ID or City is incorrect", e);
			throw new UserManagerException(
					"Error in login Please try again later", e);
		}

		if (list == null) {
			logger.warn("Account ID or City is incorrect");
			throw new UserManagerException(
					"Error in login Please check you username or password");
		}

		if (list.size() > 1) {
			logger.warn("Account ID or City dublicated Please Check, account="
					+ user.getContractNo() + " , city=" + user.getCity());
		}
		...
		for (UserAccount userAccount : list) {
			...
			if (StringUtils.isNotEmpty(user.getPassword())
				...
				if (userAccount.getStatus() != UserStatus.ACTIVE.value()) {
					...
					throw new UserManagerException(
							"Error in login This user is in active");
				}
		...
		throw new UserManagerException(
				"Error in login Please check you password");
	...
	public boolean activate(String activationString) {
		...
		try {
			...
			if (user != null
					&& activationString.equals(user.getActivationString())) {
	...
	private UserAccount getUserFromActivationLink(String string)
			throws Exception {
		...
		List<UserAccount> list = FacadeFactory.getFacade().list(
				UserAccount.class, parameters);
